...
bool text_field_ex(std::string_view label, std::string_view hint, char* buf, int buf_size, float text_padding, float line_padding, ImU32 text_col, bool centered, const ImRect& rect, bool* active, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data, bool fg = false)
{
    ...
    text_field_state* animstate = gui->anim_container(&animstate, id);
    ...
    const bool hovered = fg ? rect.Contains(g.IO.MousePos) : gui->item_hoverable(rect, id, g.LastItemData.InFlags);
    ...
    *active = render_cursor;
    ...
    if (!is_multiline)
    {
        draw->text_clipped(window->DrawList, font->get(inter_semi_bold_data, 11), rect.Min + ImVec2(text_padding, 0), rect.Max, draw->get_clr(clr->textfield.text, 1 - animstate->alpha), hint.data(), NULL, NULL, ImVec2(0.f, 0.5f));
    }
    ...
    ImVec2 draw_pos = centered ? ImVec2(rect.Min.x + (rect.GetWidth() - buf_text_size.x) / 2, rect.GetCenter().y - gui->text_size(g.Font, "A").y / 2) : ImVec2(rect.Min.x + text_padding, rect.GetCenter().y - gui->text_size(g.Font, "A").y / 2);
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const char* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (d.Overlaps(clip_rect))
                        (fg ? gui->foreground_drawlist() : window->DrawList)->AddRectFilled(d.Min, d.Max, draw->get_clr(clr->main.accent, 0.2f), 2);
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            (fg ? gui->foreground_drawlist() : window->DrawList)->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, text_col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            gui->easing(animstate->offset, cursor_screen_rect.Min.x - rect.Min.x, 20, dynamic_easing);

            if (!animstate->active_cursor)
            {
                animstate->cursor_alpha += gui->fixed_speed(6.f);

                if (animstate->cursor_alpha >= 0.9f)
                    animstate->timer += gui->fixed_speed(6.f);

                if (animstate->timer >= 3.f)
                {
                    animstate->active_cursor = true;
                    animstate->timer = 0.f;
                }
            }
            else
            {
                ...
                if (animstate->cursor_alpha <= 0.1f)
                    animstate->timer += gui->fixed_speed(6.f);

                if (animstate->timer >= 1.f)
                {
                    animstate->active_cursor = false;
                    animstate->timer = 0.f;
                }
            }
            ...
            if (cursor_screen_rect.Overlaps(clip_rect))
                draw->line((fg ? gui->foreground_drawlist() : window->DrawList), rect.Min + ImVec2(animstate->offset, line_padding), ImVec2(rect.Min.x + animstate->offset, rect.Max.y - line_padding), draw->get_clr(clr->main.text, animstate->cursor_alpha), SCALE(1));
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            (fg ? gui->foreground_drawlist() : window->DrawList)->AddText(g.Font, g.FontSize, draw_pos, text_col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
...
bool c_widgets::text_field(std::string_view name, std::string_view icon, char* buf, int size)
{
    struct field_state
    {
        bool active{ false };
    ...
    const ImRect icon_zone(total.Min, ImVec2(total.Min.x + SCALE(elements->window.padding.x) + gui->text_size(font->get(icons_data, 10), icon.data()).x, total.Max.y));
    const ImRect text_zone(ImVec2(icon_zone.Max.x, total.Min.y), ImVec2(total.Max.x, total.Max.y));

    field_state* state = gui->anim_container(&state, gui->get_window()->GetID((std::stringstream{} << name << icon).str().data()));
    ...
    gui->easing(state->icon, state->value_changed || hasContent ? clr->main.accent.Value : clr->textfield.text.Value, 12.f, dynamic_easing);

    draw->text_clipped(window->DrawList, font->get(icons_data, 11), icon_zone.Min, icon_zone.Max, draw->get_clr(state->icon), icon.data(), NULL, NULL, ImVec2(1.f, 0.5f));

    state->value_changed = state->active;


    gui->push_font(font->get(inter_semi_bold_data, 10.f));
    text_field_ex(name.data(), name, buf, size, SCALE(elements->window.padding.x), SCALE(elements->window.padding.y * 1.5), draw->get_clr(clr->main.text), false, text_zone, &state->active, 0, 0, 0);
    gui->pop_font();
