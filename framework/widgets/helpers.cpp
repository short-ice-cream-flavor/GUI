...
void c_gui::end_group()
{
    ...
    gui->item_size(group_bb.GetSize());
    gui->item_add(group_bb, 0, NULL, ImGuiItemFlags_NoTabStop);
...
{
    bool hovered = IsMouseHoveringRect(GetWindowPos(), GetWindowPos() + ImVec2(GetWindowWidth(), SCALE(elements->top_bar.height)));
    ...
    if (clicked || var->gui.update_size)
    {
        GetWindowRect(hwnd, &rc);
        MoveWindow(hwnd, rc.left + GetMouseDragDelta().x, rc.top + GetMouseDragDelta().y, GetWindowWidth(), GetWindowHeight(), FALSE);
        var->gui.update_size = false;
    }
}

void c_gui::open_url(std::string_view url)
{
    if (url.empty()) return;

#if defined(_WIN32)
    ShellExecuteA(0, "open", std::string(url).c_str(), 0, 0, SW_SHOW);
...
void c_gui::draw_decorations()
{
    ...
    draw->rect_filled(drawlist, pos, pos + size, draw->get_clr(clr->window.background), var->style.window_rounding);
    draw->rect(drawlist, pos, pos + size, draw->get_clr(clr->window.outline), var->style.window_rounding);
